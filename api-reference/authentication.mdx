...
theGPTlab uses API keys to authenticate requests. You can manage API keys using the `api-users` endpoint.
...
```bash
curl -X POST 'https://api.gptlab.cloud/api/v1<ENDPOINT>' \
